...
function BezierSim() {
  ...
  this.loadData = function (dataString) {
    ...
    for (i; i < data.length; i++) {
      ...
      points.push({
        x: width / parseFloat(point[0]),
        y: height / parseFloat(point[1]),
      });
    }
  };
...
setTimeout(function () {
  ...
  bezierSim.loadData(
    "x17.9y4.58x7.38y1.19x3.16y1.19x2.8y2.13x2.54y7.27x1.71y7.27x1.57y2.04x1.44y1.2x1.18y1.2x1.09y5.52"
  );
  bezierSim.init();
  bezierSim.start();
}, 10);
